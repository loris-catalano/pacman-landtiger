...
{
 time_semibiscroma = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 64.0f + 0.5), // 1/128
 time_biscroma = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 32.0f + 0.5), // 1/64
 time_semicroma = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 16.0f + 0.5), // 1/32
 time_croma = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 8.0f + 0.5), // 1/16
 time_semiminima = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 4.0f + 0.5), // 1/4
 time_minima = (unsigned int)(0x17D7840 * 1.3 * 1.6 / 2.0f + 0.5), // 1/2
 time_semibreve = (unsigned int)(0x17D7840 * 1.3 * 1.6 + 0.5), // 1
} NOTE_DURATION;
...
void game_over(){
  ...
  GUI_Text(90, 135, (uint8_t *) "GAME OVER", 0xFFFF, 0xF800);
}
...
void print_power_pill(){
 int seed = ((LPC_TIM_TypeDef *) ((0x40000000UL) + 0x04000) )->TC;
 srand(seed);
 ...
 while( (maze[y][x] != 2 && maze[y][x] != 0) || ((y==10||y==11||y==17||y==18) && x<=4) || ((y==10||y==11||y==17||y==18) && x>=23) || ((y>=13 && y<=15) && (x>=11 && x<=16))){
...
char compute_next_ghost_dir(){
  ...
  if(ghost_mode == 21){
   ...
   for (i = 0; i < 4; i++) {
     ...
     if (next_y >= 0 && next_y < 31 && next_x >= 0 && next_x < 28 && maze[next_y][next_x] != 1 && (next_x!=ghost_prev_x || next_y!=ghost_prev_y) ) {
...
void move_pacman(char dir, int *curr_x, int *curr_y) {
  ...
  switch(next_block) {
    ...
    if (*curr_x != next_x || *curr_y != next_y) {
     ...
     LCD_DrawBlock(0, *curr_x * 8, *curr_y * 8);

     maze[next_y][next_x] = 4;
     LCD_DrawBlock(4, next_x * 8, next_y * 8);
   }
...
void move_ghost(char dir, int *curr_x, int *curr_y) {
  ...
  //Teleport
    ...
    if(*curr_x != next_x || *curr_y != next_y) {
     ...
     LCD_DrawBlock(6, next_x * 8, next_y * 8);
  }
...
void TIMER1_IRQHandler (void)
{
 ...
 if(score == 240*10 + 6*50){
  GUI_Text(90, 135, (uint8_t *) "VICTORY!", 0xFFFF, 0x07E0);
  disable_timer(0);
  disable_timer(1);
  disable_timer(2);
  disable_timer(3);
 ...
 while(time == time_points[arr_pos] && time >0){
  print_power_pill();
  arr_pos--;
 }


 if(ghost_mode == 20 && frightened_counter<10){
  frightened_counter++;
 }
...
# 446 "Source/timer/IRQ_timer.c"
void TIMER3_IRQHandler (void)
{
 disable_timer(2);
 ((LPC_TIM_TypeDef *) ((0x40080000UL) + 0x10000) )->TC =0; // Reset Timer 2
