...
# 1 "./Source\\../sample.h" 1
# 27 "Source/GLCD/GLCD.c" 2


static uint8_t LCD_Code;
# 57 "Source/GLCD/GLCD.c"
...
# 95 "Source/GLCD/GLCD.c"
static void wait_delay(int count)
{
 while(count--);
}
# 108 "Source/GLCD/GLCD.c"
...
# 233 "Source/GLCD/GLCD.c"
static void LCD_SetCursor(uint16_t Xpos,uint16_t Ypos)
{
# 246 "Source/GLCD/GLCD.c"
  switch( LCD_Code )
  {
     default:
...
void LCD_DrawBlock(uint8_t block_type, uint16_t Xpos, uint16_t Ypos){
 ...
 switch(block_type){
  case 0:
   for(i=0; i<8; i++){
    LCD_DrawLine(Xpos, Ypos+i, Xpos+8, Ypos+i, 0x0000);
   }
   break;
  case 1:
   LCD_DrawLine(Xpos, Ypos, Xpos+7, Ypos, 0x0000);
   LCD_DrawLine(Xpos+1, Ypos+1, Xpos+7, Ypos+1, 0x001F); //top -
   LCD_DrawLine(Xpos+7, Ypos+1, Xpos+7, Ypos+7, 0x001F); //right |
   LCD_DrawLine(Xpos+1, Ypos+7, Xpos+7, Ypos+7, 0x001F); //bottom -
   LCD_DrawLine(Xpos+1, Ypos+1, Xpos+1, Ypos+7, 0x001F); //left |

   LCD_DrawLine(Xpos, Ypos, Xpos, Ypos+7, 0x0000);


   for(i=2; i<7; i++){
    LCD_DrawLine(Xpos+2, Ypos+i, Xpos+6, Ypos+i, 0x0000);
   }
   break;
  case 2: //STANDARD PILL
   LCD_DrawBlock(0, Xpos-8, Ypos-24);
   LCD_SetPoint(Xpos+4, Ypos+3, 0x7FFF);
   //LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, 0xF800); //1 -
   LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, 0x7FFF); //2 -
   //LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, 0xF800); //3 -
   LCD_SetPoint(Xpos+4, Ypos+5, 0x7FFF);
   break;
  case 3: //POWER PILL
   LCD_DrawBlock(0, Xpos-8, Ypos-24);
   LCD_DrawLine(Xpos+3, Ypos+2, Xpos+5, Ypos+2, 0x07E0);
   LCD_DrawLine(Xpos+2, Ypos+3, Xpos+6, Ypos+3, 0x07E0);
   LCD_DrawLine(Xpos+2, Ypos+4, Xpos+6, Ypos+4, 0x07E0);
   LCD_DrawLine(Xpos+2, Ypos+5, Xpos+6, Ypos+5, 0x07E0);
   LCD_DrawLine(Xpos+3, Ypos+6, Xpos+5, Ypos+6, 0x07E0);

   break;
  case 4:
   LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, 0xFFE0);
   LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, 0xFFE0);
   LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, 0xFFE0);
   LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, 0xFFE0);
   LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, 0xFFE0);
   break;
  case 6:
   LCD_DrawBlock(0, Xpos-8, Ypos-24);

   if(ghost_mode == 21){
    LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, 0xF800);
    LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, 0xF800);
    LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, 0xF800);
    LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, 0xF800);
    LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, 0xF800);
   }
   else{
    LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, 0x001F);
    LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, 0x001F);
    LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, 0x001F);
    LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, 0x001F);
    LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, 0x001F);
   }
   break;
  case 9:
   LCD_DrawBlock(0, Xpos-8, Ypos-24);

   LCD_DrawLine(Xpos+1, Ypos+4, Xpos+7, Ypos+4, 0xF81F);
   LCD_DrawLine(Xpos+1, Ypos+5, Xpos+7, Ypos+5, 0xF81F);
   break;
