...
void LCD_DrawBlock(uint8_t block_type, uint16_t Xpos, uint16_t Ypos){
	...
	switch(block_type){
		case BLACK: 
			for(i=0; i<8; i++){
				LCD_DrawLine(Xpos, Ypos+i, Xpos+8, Ypos+i, Black);
			}
			break;
		case WALL:
			LCD_DrawLine(Xpos, Ypos, Xpos+7, Ypos, Black);			
			LCD_DrawLine(Xpos+1, Ypos+1, Xpos+7, Ypos+1, Blue); //top -
			LCD_DrawLine(Xpos+7, Ypos+1, Xpos+7, Ypos+7, Blue); //right |
			LCD_DrawLine(Xpos+1, Ypos+7, Xpos+7, Ypos+7, Blue); //bottom -
			LCD_DrawLine(Xpos+1, Ypos+1, Xpos+1, Ypos+7, Blue); //left |
		
			LCD_DrawLine(Xpos, Ypos, Xpos, Ypos+7, Black);	
		
			
			for(i=2; i<7; i++){
				LCD_DrawLine(Xpos+2, Ypos+i, Xpos+6, Ypos+i, Black);
			}
			break;
		case STDPILL:	//STANDARD PILL
			LCD_DrawBlock(BLACK, Xpos-8, Ypos-24);
			LCD_SetPoint(Xpos+4, Ypos+3, Cyan);
			//LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, Red); //1 -
			LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, Cyan); //2 -
			//LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, Red); //3 -
			LCD_SetPoint(Xpos+4, Ypos+5, Cyan);
			break;
		case PWRPILL:	//POWER PILL
			LCD_DrawBlock(BLACK, Xpos-8, Ypos-24);
			LCD_DrawLine(Xpos+3, Ypos+2, Xpos+5, Ypos+2, Green); 
			LCD_DrawLine(Xpos+2, Ypos+3, Xpos+6, Ypos+3, Green); 
			LCD_DrawLine(Xpos+2, Ypos+4, Xpos+6, Ypos+4, Green); 
			LCD_DrawLine(Xpos+2, Ypos+5, Xpos+6, Ypos+5, Green); 
			LCD_DrawLine(Xpos+3, Ypos+6, Xpos+5, Ypos+6, Green); 
		
			break;
		case FPACMAN:
			LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, Yellow);
			LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, Yellow); 
			LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, Yellow); 
			LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, Yellow); 
			LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, Yellow);
			break;
		case GHOST:
			LCD_DrawBlock(BLACK, Xpos-8, Ypos-24);
		
			if(ghost_mode == CHASE_MODE){
				LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, Red);
				LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, Red); 
				LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, Red); 
				LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, Red);
				LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, Red);
			}
			else{
				LCD_DrawLine(Xpos+2, Ypos+2, Xpos+4, Ypos+2, Blue);
				LCD_DrawLine(Xpos+3, Ypos+3, Xpos+5, Ypos+3, Blue); 
				LCD_DrawLine(Xpos+3, Ypos+4, Xpos+5, Ypos+4, Blue); 
				LCD_DrawLine(Xpos+3, Ypos+5, Xpos+5, Ypos+5, Blue);
				LCD_DrawLine(Xpos+2, Ypos+6, Xpos+4, Ypos+6, Blue);
			}
			break;
		case MAGENTALINE:
			LCD_DrawBlock(BLACK, Xpos-8, Ypos-24);
		
			LCD_DrawLine(Xpos+1, Ypos+4, Xpos+7, Ypos+4, Magenta);
			LCD_DrawLine(Xpos+1, Ypos+5, Xpos+7, Ypos+5, Magenta);
			break;
